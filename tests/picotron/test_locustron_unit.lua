...
test("adding same object twice should error", function()
   ...
   assert_error(
      function() loc.add(obj, 20, 20, 8, 8) end,
      "object already in spatial hash",
      "adding same object twice should fail"
   )
end)
...
test("remove unknown object should error", function()
   ...
   assert_unknown_object_error(function() loc.del(obj) end, "deleting unknown object should fail")
end)
...
test("update unknown object should error", function()
   ...
   assert_unknown_object_error(function() loc.update(obj, 20, 30, 8, 8) end, "updating unknown object should fail")
end)
...
test("query with filter function", function()
   ...
   local results = loc.query(0, 0, 50, 50, function(obj) return obj.type == "enemy" end)
...
test("error on unknown object update", function()
   ...
   assert_unknown_object_error(function() loc.update(obj, 10, 10, 8, 8) end)
end)
...
test("error on unknown object delete", function()
   ...
   assert_unknown_object_error(function() loc.del(obj) end)
end)
...
test("query deduplication", function()
   ...
   for result_obj in pairs(results) do
      if result_obj == obj then count = count + 1 end
   end
