...
describe("Strategy Interface Foundation", function()
   ...
   describe("StrategyFactory", function()
      ...
      function MockStrategy:update_object(obj, x, y, w, h)
         if self.objects[obj] then self.objects[obj] = { x = x, y = y, w = w, h = h } end
         return obj
      end
      ...
      function MockStrategy:query_region(x, y, w, h, filter_fn)
         ...
         for obj, data in pairs(self.objects) do
            ...
            if data.x < x + w and data.x + data.w > x and data.y < y + h and data.y + data.h > y then
      ...
      function MockStrategy:get_bbox(obj)
         local data = self.objects[obj]
         if data then return data.x, data.y, data.w, data.h end
         return nil
      end

      function MockStrategy:clear() self.objects = {} end
      ...
      before_each(function()
         ...
         StrategyFactory.register_strategy("mock", MockStrategy, {
            ...
            validate_config = function(config)
               if config.invalid then return false, "Invalid configuration" end
               return true, nil
            end,
         })
      end)
      ...
      it("should handle unknown strategies gracefully", function()
         assert.has_error(function() StrategyFactory.create_strategy("unknown_strategy") end) -- Just check that it throws an error, don't match exact message
      end)
      ...
      it("should validate configurations", function()
         ...
         assert.truthy(err3 and string.find(err3, "Unknown strategy"))
      end)
      ...
      it("should get strategy metadata", function()
         ...
         assert.truthy(metadata)
         --[[@diagnostic disable:need-check-nil]]
         local desc = metadata.description
         local optimal = metadata.optimal_for
         assert(desc, "description should not be nil")
         assert.equals("Mock strategy for testing", desc)
         assert(optimal, "optimal_for should not be nil")
         assert.same({ "testing" }, optimal)
   ...
   describe("Convenience API", function()
      ...
      before_each(function() StrategyFactory.register_strategy("mock", MockStrategy, {}) end)
      ...
      it("should handle invalid options gracefully", function()
         assert.has_error(function() strategy_module.create_strategy("42") end) -- Just check that it throws an error
      end)
   end)
