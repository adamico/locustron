...
function BenchmarkCLI.parse_args(args)
   ...
   while i <= #args do
      ...
      elseif arg == "--strategies" or arg == "-s" then
         i = i + 1
         if i <= #args then config.strategies = BenchmarkCLI.parse_list(args[i]) end
      elseif arg == "--scenarios" or arg == "-c" then
         i = i + 1
         if i <= #args then config.scenarios = BenchmarkCLI.parse_list(args[i]) end
      elseif arg == "--iterations" or arg == "-i" then
         i = i + 1
         if i <= #args then config.iterations = tonumber(args[i]) or config.iterations end
      elseif arg == "--output" or arg == "-o" then
         i = i + 1
         if i <= #args then config.output_format = args[i] end
   ...
   if #config.strategies == 0 then config.strategies = { "fixed_grid" } end

   if #config.scenarios == 0 then config.scenarios = { "uniform", "clustered" } end
...
function BenchmarkCLI.run_benchmark(config)
   ...
   for _, scenario_name in ipairs(config.scenarios) do
      if config.verbose then print("Running scenario: " .. scenario_name) end

      results[scenario_name] = {}

      for _, count in ipairs(object_counts) do
         if config.verbose then print("  Object count: " .. count) end
   ...
   for scenario_name, scenario_results in pairs(results) do
      for count, count_results in pairs(scenario_results) do
         for strategy_name, strategy_results in pairs(count_results) do
            ...
            strategy_totals[strategy_name].add_time = strategy_totals[strategy_name].add_time
               + strategy_results.add_time
            ...
            strategy_totals[strategy_name].accuracy = strategy_totals[strategy_name].accuracy
      ...
      for scenario_name, scenario_results in pairs(results) do
         ...
         for count, count_results in pairs(scenario_results) do
            ...
            for strategy_name, strategy_results in pairs(count_results) do
               print(
                  string.format(
                     "    %s: %.3fms add, %.3fms query, %.1fKB memory, %.1f%% accuracy",
                     strategy_name,
...
function BenchmarkCLI.output_json(results, profiler)
   ...
   if profiler then output.profiling_enabled = true end
...
function BenchmarkCLI.validate_config(config)
   ...
   for _, strategy in ipairs(config.strategies) do
      ...
      if not found then table.insert(errors, "Unknown strategy: " .. strategy) end
   end
   ...
   for _, scenario in ipairs(config.scenarios) do
      ...
      if not found then table.insert(errors, "Unknown scenario: " .. scenario) end
   end

   -- Validate iterations
   if config.iterations < 1 then table.insert(errors, "Iterations must be >= 1") end
...
function BenchmarkCLI.main(args)
   ...
   if config.verbose then print("\nBenchmark completed successfully!") end
end
