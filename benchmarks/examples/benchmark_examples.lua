...
for _, scenario_name in ipairs(scenarios) do
   ...
   print(
      string.format(
         "%s scenario: %.3f ms query, %.1f KB memory",
         scenario_name,
...
for _, grid_size in ipairs(grid_sizes) do
   local grid_results = benchmark_suite:benchmark_strategy("fixed_grid", test_objects)

   print(
      string.format(
         "  %dx%d grid: %.3f ms query, %.1f KB memory, %.1f%% accuracy",
         grid_size,
         grid_size,
...
for _, count in ipairs(object_counts) do
   ...
   print(
      string.format(
         "  %d objects: %.1f KB (%.1f bytes/object)",
         count,
...
for _, density in ipairs(densities) do
   ...
   print(
      string.format(
         "  %s density (%d objects): %.2f%% accuracy",
         density.name,
         density.count,
...
for _, query_info in ipairs(query_sizes) do
   ...
   print(
      string.format(
         "  %s queries (%dx%d): [Would test %dx%d query performance]",
         query_info.name,
         query_info.size,
         query_info.size,
         query_info.size,
