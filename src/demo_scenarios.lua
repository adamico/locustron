...
function DemoScenarios.space_battle(config)
   return SpaceBattleScenario.new(config)
end

-- Scenario: Platformer Level
-- Objects in a bounded level with some clustering around platforms
function DemoScenarios.platformer(config)
   return PlatformerScenario.new(config)
end

-- Scenario: Dynamic Ecosystem
-- Objects spawn, move, and die creating changing density patterns
function DemoScenarios.dynamic_ecosystem(config)
   return DynamicEcosystemScenario.new(config)
end
