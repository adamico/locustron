...
function DebugConsole:register_builtin_commands()
   ...
   self:register_command(
      "inspect",
      function(args) return self:inspect_strategy(args[1]) end,
      "Inspect spatial strategy (inspect <strategy_name>)"
   )

   self:register_command(
      "objects",
      function(args) return self:list_objects(args[1]) end,
      "List objects in strategy (objects [filter])"
   )

   self:register_command(
      "cells",
      function(args) return self:list_cells(args[1]) end,
      "List occupied cells (cells [limit])"
   )

   -- Query commands
   self:register_command(
      "query",
      function(args) return self:execute_query(args) end,
      "Execute spatial query (query <x> <y> <w> <h>)"
   )

   self:register_command(
      "benchmark",
      function(args) return self:run_benchmark(args) end,
      "Run performance benchmark (benchmark <iterations>)"
   )

   -- Performance commands
   self:register_command("perf", function(args) return self:get_performance_report() end, "Show performance report")

   self:register_command(
      "bottlenecks",
      function(args) return self:get_bottlenecks(args[1]) end,
      "Show performance bottlenecks (bottlenecks [count])"
   )

   -- Visualization commands
   self:register_command(
      "show",
      function(args) return self:toggle_visualization(args[1]) end,
      "Toggle visualization (show <structure|objects|queries|performance>)"
   )

   self:register_command("zoom", function(args) return self:set_zoom(args[1]) end, "Set zoom level (zoom <level>)")

   -- Configuration commands
   self:register_command("config", function(args) return self:show_config() end, "Show current configuration")

   self:register_command(
      "set",
      function(args) return self:set_config(args) end,
   ...
   self:register_command(
      "history",
      function(args) return self:show_history(args[1]) end,
      "Show command history (history [count])"
   )

   self:register_command("echo", function(args) return table.concat(args, " ") end, "Echo arguments")
end
...
function DebugConsole:execute_command(command_line)
   if not command_line or command_line == "" then return "" end
   ...
   if #self.history > self.config.max_history then deli(self.history, 1) end

   -- Parse command
   local args = self:parse_command_line(command_line)
   local cmd_name = deli(args, 1)

   if not cmd_name then return "Error: Empty command" end
   ...
   if not success then result = string.format("Error executing command '%s': %s", cmd_name, result) end
...
function DebugConsole:parse_command_line(command_line)
   ...
   if current_arg ~= "" then add(args, current_arg) end
...
function DebugConsole:add_output(text)
   ...
   if #self.output_buffer > self.config.max_output_lines then deli(self.output_buffer, 1) end
end

--- Clear the output buffer
function DebugConsole:clear_output() self.output_buffer = {} end
...
function DebugConsole:inspect_strategy(strategy_name)
   if not strategy_name then return "Usage: inspect <strategy_name>" end
...
function DebugConsole:list_objects(filter)
   ...
   if count == 0 then add(lines, "  No objects found") end
...
function DebugConsole:execute_query(args)
   if #args < 4 then return "Usage: query <x> <y> <w> <h>" end

   local x, y, w, h = tonumber(args[1]), tonumber(args[2]), tonumber(args[3]), tonumber(args[4])
   if not (x and y and w and h) then return "Error: Invalid numeric arguments" end

   if not self.current_strategy or not self.current_strategy.query_region then return "No queryable strategy loaded" end
   ...
   if self.visualization_system then self.visualization_system:add_query(x, y, w, h, result_count) end
   ...
   return string.format(
      "Query (%.1f,%.1f,%.1f,%.1f) returned %d results in %.3fms",
      x,
      y,
      w,
      h,
...
function DebugConsole:run_benchmark(args)
   ...
   if not self.performance_profiler then return "Performance profiler not available" end
   ...
   local stats = self.performance_profiler.stats
   return string.format(
      "Benchmark completed: %d queries, avg %.3fms, %.1f queries/sec",
      iterations,
      stats.average_query_time * 1000,
...
function DebugConsole:toggle_visualization(option)
   if not self.visualization_system then return "Visualization system not available" end

   if option == "structure" then
      self.visualization_system.show_structure = not self.visualization_system.show_structure
      return string.format("Structure visualization: %s", self.visualization_system.show_structure and "ON" or "OFF")
   elseif option == "objects" then
      self.visualization_system.show_objects = not self.visualization_system.show_objects
      return string.format("Object visualization: %s", self.visualization_system.show_objects and "ON" or "OFF")
   elseif option == "queries" then
      self.visualization_system.show_queries = not self.visualization_system.show_queries
      return string.format("Query visualization: %s", self.visualization_system.show_queries and "ON" or "OFF")
...
function DebugConsole:set_zoom(level_str)
   ...
   if not level or level <= 0 then return "Error: Invalid zoom level" end
...
function DebugConsole:set_config(args)
   if #args < 2 then return "Usage: set <key> <value>" end
   ...
   if self.config[key] == nil then return string.format("Error: Unknown configuration key '%s'", key) end
...
function DebugConsole:show_history(count_str)
   ...
   if #self.history == 0 then add(lines, "  No commands in history") end
...
function DebugConsole:get_object_count()
   if not self.current_strategy or not self.current_strategy.objects then return 0 end
...
function DebugConsole:get_time() return time and time() end
...
function DebugConsole:set_strategy(strategy, strategy_name)
   ...
   if strategy then strategy._strategy_name = strategy_name end
end

--- Set visualization system integration
--- @param visualization_system table VisualizationSystem instance
function DebugConsole:set_visualization_system(visualization_system) self.visualization_system = visualization_system end

--- Set performance profiler integration
--- @param performance_profiler table PerformanceProfiler instance
function DebugConsole:set_performance_profiler(performance_profiler) self.performance_profiler = performance_profiler end

--- Get output buffer for display
--- @return table Output buffer
function DebugConsole:get_output() return self.output_buffer end
